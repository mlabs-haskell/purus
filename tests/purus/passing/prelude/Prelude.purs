                           ...
                           (Builtin.mkCons (serializeValue rec.mint)
                              (Builtin.mkCons (serializeList serializeDCert rec.dCert)
                                 (Builtin.mkCons (serializeAssocMap serializeStakingCredential serializeInt rec.wdrl)
                                    (Builtin.mkCons (serializeInterval serializePOSIXTime rec.validTimeRange)
...
serializeTxOutRef :: Builtin.TxOutRef -> Builtin.BuiltinData
...
deserializeTxOutRef :: Builtin.BuiltinData -> Builtin.TxOutRef
...
takeList count ell = 
  ...
  then Nil
          ...
          Nil -> Nil
          Cons x xs -> Cons x (takeList (Builtin.subtractInteger 1) xs)
...
dropList count ell = 
  ...
  then ell
           ...
           Nil -> Nil
           Cons _ xs -> dropList (Builtin.subtractInteger 1) xs
