...
import Language.PureScript.Names (coerceProperName, runProperName)
      ...
      ctrs st (DataCtor _ name fields) tl = do
        ...
        case tl of
          ...
          (st', ctor) : tl' -> do
            ...
            pure $
              AST.DataConstructorDeclaration (sourceAnnCommented fileName st (nameTok name)) (nameValue name) (zip ctrFields fields')
                : rest
    ...
    let
      ...
      goTvBind stok = \case
        ...
        TypeVarName (_, nm) -> do
          ...
          internalError $
            "Error in instance declaration near "
              <> prettyRange (srcTokenRange stok)
              <> "\n  Missing kind annotation for Type Var '"
              <> nm'
              <> "'"
  ...
  where
    ...
    findInstanceAnn cls args =
      uncurry (sourceAnnCommented fileName) $
        if null args
          then qualRange cls
          else (fst $ qualRange cls, snd $ typeRange $ last args)
