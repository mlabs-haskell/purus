...
import Prelude hiding (exp, exponent, lex, lines)
  ...
  where
    k0 acc = do
      ...
      if Text.null lines
        ...
        else k1 acc' (goWs [] $ Text.unpack lines)
...
token = peek >>= maybe (pure TokEof) k0
  where
    k0 ch1 = case ch1 of
      ...
      '"' -> next *> string
      _
        | Char.isDigit ch1 -> restore (== ErrNumberOutOfRange) (next *> number ch1)
              ...
              case Text.uncons inp2 of
                Just (')', inp3) ->
                  case chs of
                    ...
                    _
                      | isReservedSymbol chs -> kerr inp ErrReservedSymbol
        ...
        Just 'e' -> do
          ...
          integer >>= \case
            Just (raw, chs) -> do
              let
                int
                  | neg = negate $ digitsToInteger chs
