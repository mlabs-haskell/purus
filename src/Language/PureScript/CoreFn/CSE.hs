...
import Language.PureScript.Names (Ident (..), ModuleName, ProperName (..), Qualified (..), QualifiedBy (..), freshIdent, runIdent, toMaybeModuleName, pattern ByNullSourcePos)
...
replaceLocals m = if M.null m then identity else map f'
  where
...
getNewBindsAsLet ::
  ...
  m (Expr Ann) ->
  m (Expr Ann)
getNewBindsAsLet = fmap (uncurry go) . getNewBinds
  where
    go bs =
      if null bs
        then identity
...
optimizeCommonSubexpressions mn =
  ...
  where
    ...
    handleExpr =
      discuss (ifM (shouldFloatExpr . fst) (floatExpr topLevelQB) pure) . \case
    ...
    handleCaseAlternative (CaseAlternative bs x) =
      CaseAlternative bs <$> do
    ...
    handleBinds isTopLevel = foldr go . fmap pure
      where
