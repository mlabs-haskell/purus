...
exprToCoreFn mn ss Nothing accessor@(A.Accessor name v) = do
  traceM $ "accessor: " <> renderValue 100 accessor
...
exprToCoreFn _ _ t lam@(A.Abs _ _) =
  internalError $
    "Abs with Binder argument was not desugared before exprToCoreFn: \n"
      ...
      <> "\n"
      <> show lam
exprToCoreFn mn ss _ app@(A.App fun arg)
...
exprToCoreFn _ _ (Just ty) (A.Var ss ident)
  | globalOrGenerated ident = do -- ident@(Qualified _ (GenIdent _ _))) =
      envTy <- gets checkEnv >>= pure . flip lookupValue ident
      traceM $ "var1: " <> T.unpack (showQualified runIdent ident)
                   <> "\nty: " <> prettyStr ty
                   <> "\nenvTy: " <> prettyStr (view _1 <$> envTy)
                   <> "\n------------------\n"
      wrapTrace ("exprToCoreFn VAR (gen) " <> show ident) $
        gets checkEnv >>= \env ->
          pure $ Var (ss, [], getValueMeta env ident) (purusTy ty) ident
 where
   globalOrGenerated = \case
     Qualified (ByModuleName _) _ -> True
     Qualified _ (GenIdent{}) -> True
     _ -> False
...
-- REVIEW: NEW NOTE(12/18): Not sure if the previous is true. It seems like we *do* want to prefer the passed-in type
--                          for (e.g.) zero argument constructors or functions w/ a polymorphic type...
exprToCoreFn _ _ mty (A.Var ss ident) = do
  traceM $ "var2: " <> T.unpack (showQualified runIdent ident) <> "\nmty: " <> prettyStr mty <> "\n----------------\n"
...
exprToCoreFn _ _ Nothing ctor@(A.Constructor ss name) = do
  env <- getEnv
  let (_,_,ctorTy,_) = lookupConstructor env name
      ctorMeta = getConstructorMeta env name
  pure $ Var (ss,[],Just ctorMeta) ctorTy (properToIdent <$> name)
...
exprToCoreFn mn ss (Just ty) tv1@(A.TypedValue _ v ty2) = do
  wrapTrace "exprToCoreFn TV1" $ do
    traceM $ "val: " <> renderValue 100 v
    traceM $ "annTy: " <> prettyStr ty2
    traceM $ "supplied ty: " <> prettyStr ty
    exprToCoreFn mn ss (Just ty) v
-- If we encounter a TypedValue w/o a supplied type, we use the annotated type
exprToCoreFn mn ss Nothing (A.TypedValue _ v ty) = do
...
altToCoreFn mn ss ret boundTypes (A.CaseAlternative bs vs) = wrapTrace "altToCoreFn" $ do
  ...
  traceM $ "BTYPES: " <> prettyStr (M.toList $ view _2 <$> bTypes)
  ege <- go toBind vs
