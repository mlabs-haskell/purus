...
desugarCore :: Expr Ann -> DS (Exp PurusType (FVar PurusType))
...
desugarAlt :: CaseAlternative Ann -> DS (Alt PurusType (Exp PurusType) (FVar PurusType))
...
desugarBinds :: [Bind Ann] -> DS [[(FVar PurusType, Exp PurusType (FVar PurusType))]]
desugarBinds [] = pure []
...
desugarLit :: Literal (Expr Ann) -> DS (Lit (Exp PurusType (FVar PurusType)))
...
desugarLit (ObjectLiteral object) =
  ObjectL <$> desugarObjectMembers object

-- this looks like existing monadic combinator but I couldn't find it
desugarObjectMembers :: [(field, Expr Ann)] -> DS [(field, Exp PurusType (FVar PurusType))]
