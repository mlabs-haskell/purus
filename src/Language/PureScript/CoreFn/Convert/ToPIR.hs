...
toPIR f = \case
 ...
 where
   mkBinding :: (Name, Scope BVar Exp x) -> State ConvertState (Binding TyName Name DefaultUni DefaultFun ())
...
litToPIR :: forall x. (x -> Var BVar FVar) -> Lit (Exp x) -> State ConvertState  PIRTerm
litToPIR f = \case
