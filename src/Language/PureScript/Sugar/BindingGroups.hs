...
import Language.PureScript.Names (Ident, ModuleName, ProperName, ProperNameType (..), Qualified (..), QualifiedBy (..), coerceProperName, pattern ByNullSourcePos)
...
flattenBindingGroups = concatMap go
  where
    ...
    go (BindingGroupDeclaration ds) =
      NEL.toList $
        fmap
          ( \((sa, ident), nameKind, val) ->
              ValueDecl sa ident nameKind [] [MkUnguarded val]
          )
          ds
    go other = [other]
