...
import Data.List (find, foldl', unzip5)
import Language.PureScript.AST (Binder (..), CaseAlternative (..), DataConstructorDeclaration (..), Declaration (..), Expr (..), Module (..), SourceSpan (..), TypeInstanceBody (..), pattern MkUnguarded, pattern ValueDecl)
...
import Language.PureScript.Names (Ident (..), ModuleName, ProperName (..), ProperNameType (..), Qualified (..), QualifiedBy (..), freshIdent, pattern ByNullSourcePos)
...
deriveInstance mn ds decl =
  case decl of
    TypeInstanceDeclaration sa@(ss, _) na ch idx nm deps className tys DerivedInstance ->
      let
...
deriveGenericRep ss mn tyCon tyConArgs =
      ...
      let rep = toRepTy reps
          inst
            | null reps =
                ...
                , ValueDecl (ss', []) (Ident "from") Public [] $
                    unguarded $
                      lamCase
                        x
                        [ CaseAlternative
  ...
  where
    ...
    sumBinders =
      select
        (ConstructorBinder ss Libs.C_Inl . pure)
        (ConstructorBinder ss Libs.C_Inr . pure)

    sumExprs :: Int -> [Expr -> Expr]
    sumExprs =
      select
        (App (Constructor ss Libs.C_Inl))
    ...
    makeInst (DataConstructorDeclaration _ ctorName args) = do
      ...
      return
        ( srcTypeApp
            ( srcTypeApp
                (srcTypeConstructor Libs.Constructor)
                (srcTypeLevelString $ mkString (runProperName ctorName))
            )
            ctorTy
        , CaseAlternative
            [ConstructorBinder ss Libs.C_Constructor [matchProduct]]
            (unguarded (foldl' App (Constructor ss (Qualified (ByModuleName mn) ctorName)) ctorArgs))
        , CaseAlternative
            [ConstructorBinder ss (Qualified (ByModuleName mn) ctorName) matchCtor]
    ...
    makeProduct args = do
      ...
      pure
        ( foldr1 (\f -> srcTypeApp (srcTypeApp (srcTypeConstructor Libs.Product) f)) tys
    ...
    makeArg arg = do
      ...
      pure
        ( srcTypeApp (srcTypeConstructor Libs.Argument) arg
