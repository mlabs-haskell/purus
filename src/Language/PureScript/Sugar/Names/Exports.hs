...
import Data.List (groupBy, intersect, sortOn)
  ...
  where
    ...
    isPseudoModule = testQuals M.keys
      where
        ...
        testQuals f mn' =
          any (isQualifiedWith mn') (f (importedTypes imps))
      ...
      where
        ...
        checkDcon tcon dcons dcon =
          unless (dcon `elem` dcons)
            . throwError
            . errorMessage' ss
            $ UnknownExportDataConstructor tcon dcon
    ...
    filterExport toName get fromExps result ref
      | Just name <- get ref =
          case name `M.lookup` fromExps exps of
            ...
            Just source'
              | mn == exportSourceDefinedIn source' ->
