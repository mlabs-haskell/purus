...
import Language.PureScript.AST (Declaration (..), ErrorMessageHint (..), Expr (..), GuardedExpr (..), KindSignatureFor (..), Module (..), RoleDeclarationData (..), TypeDeclarationData (..), TypeInstanceBody (..), declSourceSpan, everywhereOnValuesTopDownM, pattern MkUnguarded, pattern ValueDecl)
...
desugarTypeDeclarationsModule (Module modSS coms name ds exps) =
  ...
  where
    ...
    checkRoleDeclarations (Just (RoleDeclaration (RoleDeclarationData _ name' _))) ((RoleDeclaration RoleDeclarationData {..}) : _)
      | name' == rdeclIdent =
