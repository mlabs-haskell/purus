        ...
        unzip
          <$> forM
            ...
            ( \e -> do
                ...
                scrut_id <- freshIdent'
                pure
                  ( mkTyped $ Var ss (Qualified ByNullSourcePos scrut_id)
    ...
    desugarGuard (ConditionGuard c : gs) e match_failed
      ...
      | otherwise =
          Case
            [c]
            ( CaseAlternative
                [LiteralBinder ss (BooleanLiteral True)]
                [MkUnguarded (desugarGuard gs e match_failed)]
                : match_failed 1
            )
    ...
    desugarAltOutOfLine alt_binder rem_guarded rem_alts mk_body
      | Just rem_case <- mkCaseOfRemainingGuardsAndAlts = do
          ...
          let
            ...
            goto_rem_case :: Expr
            goto_rem_case =
              mkType (Var ss (Qualified ByNullSourcePos rem_case_id))
    ...
    validate c@(Case vs alts) = do
      ...
      unless (null alts') $
        throwError . MultipleErrors $
          fmap (altError l) (caseAlternativeBinders <$> alts')
      return c
...
makeCaseDeclaration ss ident alternatives = do
  ...
  args <-
    if allUnique (catMaybes argNames)
