...
import Language.PureScript.AST (Binder (..), CaseAlternative (..), Declaration, DoNotationElement (..), Expr (..), Module (..), SourceSpan, WhereProvenance (..), binderNames, declSourceSpan, everywhereOnValuesM, pattern MkUnguarded, pattern ValueDecl)
...
import Language.PureScript.Names (Ident (..), ModuleName, Qualified (..), byMaybeModuleName, freshIdent', pattern ByNullSourcePos)
...
desugarDo d =
  ...
  where
    ...
    go _ _ (DoNotationBind b _ : _)
      | First (Just ident) <- foldMap fromIdent (binderNames b) =
