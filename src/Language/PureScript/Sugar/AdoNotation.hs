...
import Language.PureScript.AST (Binder (..), CaseAlternative (..), Declaration, DoNotationElement (..), Expr (..), Module (..), SourceSpan, WhereProvenance (..), declSourceSpan, everywhereOnValuesM, pattern MkUnguarded)
    ...
    go ss (yield, args) (DoNotationBind binder val) = do
      ...
      let abs =
            Abs
              (VarBinder ss ident)
              ( Case
                  [Var ss (Qualified ByNullSourcePos ident)]
