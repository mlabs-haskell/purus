...
import Protolude (foldl', sortOn, swap)
import Prelude
...
import Language.PureScript.Names (Ident, OpName, OpNameType (..), ProperName, ProperNameType (..), Qualified (..), QualifiedBy (..), pattern ByNullSourcePos)
...
externsEnv env ExternsFile {..} = do
  ...
  where
    ...
    localExportSource =
      ExportSource
        { exportSourceDefinedIn = efModuleName
