...
import Language.PureScript.AST (Associativity, Declaration (..), DeclarationRef (..), Fixity (..), ImportDeclarationType, Module (..), NameSource (..), Precedence, SourceSpan, getTypeOpRef, getValueOpRef, pattern TypeFixityDeclaration, pattern ValueFixityDeclaration)
    ...
    toExternsDeclaration (TypeRef _ pn dctors) =
      case Qualified (ByModuleName mn) pn `M.lookup` types env of
        ...
        Just (kind, tk@(DataType _ _ tys)) ->
          EDType pn kind tk
            : [ EDDataConstructor dctor dty pn ty args
