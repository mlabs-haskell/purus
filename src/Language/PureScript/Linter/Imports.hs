...
import Data.List (find, groupBy, intersect, sort, sortOn, (\\))
    ...
    unless (length mnis == 1) $ do
      ...
      for_ implicits $ \(ss, _, mni) -> do
        ...
        unless (null usedRefs)
          . tell
          . errorMessage' ss
          $ ImplicitQualifiedImport mni mnq
...
findUsedRefs ss env mni qn names =
  let
   ...
   in
    sort $ classRefs ++ typeOpRefs ++ typesRefs ++ valueRefs ++ valueOpRefs
  where
    reconstructTypeRefs ::
      [ProperName 'ConstructorName] ->
