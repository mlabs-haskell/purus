...
import Language.PureScript.Ide.Types (Ide, IdeDeclaration (..), IdeType (..), Match (..), Success (..), ideDtorName, ideDtorTypeName, ideTCName, ideTypeName, ideTypeOpName, ideValueOpName, _IdeDeclModule)
...
addImportForIdentifier fp ident qual filters = do
  ...
  case matches of
    ...
    ms@[Match (m1, d1), Match (m2, d2)] ->
      if m1 /= m2
        then -- If the modules don't line up we just ask the user to specify the
        -- module
          pure (Left ms)
