...
import Data.List (dropWhileEnd, nubBy, partition, sortOn, transpose, uncons)
...
data SimpleErrorMessage
  ...
  | NoInstanceFound
      -- | constraint that could not be solved
      SourceConstraint
      -- | a list of instances that stopped further progress in instance chains due to ambiguity
      [Qualified (Either SourceType Ident)]
      -- | whether eliminating unknowns with annotations might help or if visible type applications are required
      UnknownsHint
...
replaceUnknowns = everywhereOnTypesTopDownM replaceTypes
  where
  ...
  where
    ...
    prettyPrintErrorMessage typeMap (ErrorMessage hints simple) =
      ...
      where
        ...
        typeInformation
          | not (null types) = Just $ Box.hsep 1 Box.left [line "where", paras types]
          | otherwise = Nothing
          where
            types :: [Box.Box]
            types =
              map skolemInfo (M.elems (umSkolemMap typeMap))
          ...
          paras
            ...
            , line $
                if isBuiltinModuleName mn
                  then "Module names in the Prim namespace are reserved for built-in modules, but this version of the compiler does not provide module " <> markCode (runModuleName mn) <> ". You may be able to fix this by updating your compiler to a newer version."
                  else "Make sure the source file exists, and that it has been provided as an input to the compiler."
            ]
        ...
        renderSimpleErrorMessage (ErrorParsingFFIModule path extra) =
          paras $
            [ line "Unable to parse foreign module:"
            , indent . lineS $ path
            ]
        ...
        renderSimpleErrorMessage (UnnecessaryFFIModule mn path) =
          paras
            [ line $ "An unnecessary foreign module implementation was provided for module " <> markCode (runModuleName mn) <> ": "
            , indent . lineS $ path
        ...
        renderSimpleErrorMessage (MissingFFIImplementations mn idents) =
          paras
            [ line $ "The following values are not defined in the foreign module for module " <> markCode (runModuleName mn) <> ": "
            , indent . paras $ map (line . runIdent) idents
            ]
        renderSimpleErrorMessage (UnusedFFIImplementations mn idents) =
          paras
            [ line $ "The following definitions in the foreign module for module " <> markCode (runModuleName mn) <> " are unused: "
            , indent . paras $ map (line . runIdent) idents
            ]
        ...
        renderSimpleErrorMessage (DeprecatedFFICommonJSModule mn path) =
          paras
            [ line $ "A CommonJS foreign module implementation was provided for module " <> markCode (runModuleName mn) <> ": "
            , indent . lineS $ path
        ...
        renderSimpleErrorMessage (UnsupportedFFICommonJSExports mn idents) =
          paras
            [ line $ "The following CommonJS exports are not supported in the ES foreign module for module " <> markCode (runModuleName mn) <> ": "
            , indent . paras $ map line idents
            ]
        renderSimpleErrorMessage (UnsupportedFFICommonJSImports mn mids) =
          paras
            [ line $ "The following CommonJS imports are not supported in the ES foreign module for module " <> markCode (runModuleName mn) <> ": "
        ...
        renderSimpleErrorMessage (UnknownName name@(Qualified (BySourcePos _) (IdentName (Ident i))))
          | i `elem` [C.S_bind, C.S_discard] =
        ...
        renderSimpleErrorMessage (UnknownImport mn name) =
          paras
            [ line $ "Cannot import " <> printName (Qualified ByNullSourcePos name) <> " from module " <> markCode (runModuleName mn)
        ...
        renderSimpleErrorMessage (ScopeConflict nm ms) =
          paras
            [ line $ "Conflicting definitions are in scope for " <> printName (Qualified ByNullSourcePos nm) <> " from the following modules:"
            , indent $ paras $ map (line . markCode . runModuleName) ms
            ]
        renderSimpleErrorMessage (ScopeShadowing nm exmn ms) =
          paras
            [ line $ "Shadowed definitions are in scope for " <> printName (Qualified ByNullSourcePos nm) <> " from the following open imports:"
            , indent $ paras $ map (line . markCode . ("import " <>) . runModuleName) ms
            , line $
                "These will be ignored and the " <> case exmn of
        ...
        renderSimpleErrorMessage (DuplicateTypeClass pn ss) =
          paras
            [ line ("Type class " <> markCode (runProperName pn) <> " has been defined multiple times:")
            , indent $ line $ displaySourceSpan relPath ss
            ]
        renderSimpleErrorMessage (DuplicateInstance pn ss) =
          paras
            [ line ("Instance " <> markCode (showIdent pn) <> " has been defined multiple times:")
            , indent $ line $ displaySourceSpan relPath ss
            ]
        ...
        renderSimpleErrorMessage (CycleInTypeSynonym names) =
          ...
          where
            cycleError = case names of
              ...
              _ ->
                [ line " A cycle appears in a set of type synonym definitions:"
        ...
        renderSimpleErrorMessage (CycleInKindDeclaration names) =
          paras
            [ line "A cycle appears in a set of kind declarations:"
            , indent $ line $ "{" <> T.intercalate ", " (markCode . runProperName . disqualify <$> NEL.toList names) <> "}"
        ...
        renderSimpleErrorMessage (PartiallyAppliedSynonym name) =
          paras
            [ line $ "Type synonym " <> markCode (showQualified runProperName name) <> " is partially applied."
            , line "Type synonyms must be applied to all of their type arguments."
            ]
        renderSimpleErrorMessage (EscapedSkolem name Nothing ty) =
          paras
            [ line $ "The type variable " <> markCode name <> " has escaped its scope, appearing in the type"
            , markCodeBox $ indent $ prettyType ty
            ]
        ...
        renderSimpleErrorMessage (NoInstanceFound (Constraint _ C.Fail _ [ty] _) _ _)
          | Just box <- toTypelevelString ty =
              paras
                [ line "Custom error:"
                , indent box
                ]
        ...
        renderSimpleErrorMessage (DuplicateLabel l expr) =
          paras $
            [line $ "Label " <> markCode (prettyPrintLabel l) <> " appears more than once in a row type."]
              <> foldMap
                ( \expr' ->
        ...
        renderSimpleErrorMessage (ExpectedType ty kind) =
          paras
            [ line $ "In a type-annotated expression " <> markCode "x :: t" <> ", the type " <> markCode "t" <> " must have kind " <> markCode (runProperName . disqualify $ C.Type) <> "."
        ...
        renderSimpleErrorMessage (IncorrectConstructorArity nm expected actual) =
          paras
            [ line $ "Data constructor " <> markCode (showQualified runProperName nm) <> " was given " <> T.pack (show actual) <> " arguments in a case expression, but expected " <> T.pack (show expected) <> " arguments."
        ...
        renderSimpleErrorMessage (OrphanInstance nm cnm nonOrphanModules ts) =
          paras
            [ line $ "Orphan instance" <> prettyPrintPlainIdent nm <> " found for "
            , markCodeBox $
                indent $
                  Box.hsep
                    1
        ...
        renderSimpleErrorMessage (TransitiveExportError x ys) =
          paras
            [ line $ "An export for " <> markCode (prettyPrintExport x) <> " requires the following to also be exported: "
            , indent $ paras $ map (line . markCode . prettyPrintExport) ys
            ]
        renderSimpleErrorMessage (TransitiveDctorExportError x ctors) =
          paras
            [ line $ "An export for " <> markCode (prettyPrintExport x) <> " requires the following data constructor" <> (if length ctors == 1 then "" else "s") <> " to also be exported: "
            , indent $ paras $ map (line . markCode . runProperName) ctors
            ]
        renderSimpleErrorMessage (HiddenConstructors x className) =
          paras
            [ line $ "An export for " <> markCode (prettyPrintExport x) <> " hides data constructors but the type declares an instance of " <> markCode (showQualified runProperName className) <> "."
        ...
        renderSimpleErrorMessage (HoleInferredType name ty ctx ts) =
          let
            maxTSResults = 15
            tsResult = case ts of
              Just TSAfter {tsAfterIdentifiers = idents}
                | not (null idents) ->
                    let
        ...
        renderSimpleErrorMessage (MissingTypeDeclaration ident ty) =
          paras
            [ line $ "No type declaration was provided for the top-level declaration of " <> markCode (showIdent ident) <> "."
        ...
        renderSimpleErrorMessage (MissingKindDeclaration sig name ty) =
          let sigKw = prettyPrintKindSignatureFor sig
           in paras
                [ line $ "The inferred kind for the " <> sigKw <> " declaration " <> markCode (runProperName name) <> " contains polymorphic kinds."
        ...
        renderSimpleErrorMessage IncompleteExhaustivityCheck =
          paras
            [ line "An exhaustivity check was abandoned due to too many possible cases."
        ...
        renderSimpleErrorMessage msg@(UnusedExplicitImport mn names _ _) =
          paras
            [ line $ "The import of module " <> markCode (runModuleName mn) <> " contains the following unused references:"
        ...
        renderSimpleErrorMessage (IntOutOfRange value backend lo hi) =
          paras
            [ line $ "Integer value " <> markCode (T.pack (show value)) <> " is out of range for the " <> backend <> " backend."
        ...
        renderSimpleErrorMessage msg@(ImplicitQualifiedImport importedModule asModule _) =
          paras
            [ line $ "Module " <> markCode (runModuleName importedModule) <> " was imported as " <> markCode (runModuleName asModule) <> " with unspecified imports."
        ...
        renderSimpleErrorMessage msg@(ImplicitImport mn _) =
          paras
            [ line $ "Module " <> markCode (runModuleName mn) <> " has unspecified imports, consider using the explicit form: "
            , indent $ line $ markCode $ showSuggestion msg
            ]
        renderSimpleErrorMessage msg@(HidingImport mn _) =
          paras
            [ line $ "Module " <> markCode (runModuleName mn) <> " has unspecified imports, consider using the inclusive form: "
            , indent $ line $ markCode $ showSuggestion msg
            ]
        ...
        renderSimpleErrorMessage (InvalidOperatorInBinder op fn) =
          paras
            [ line $ "Operator " <> markCode (showQualified showOp op) <> " cannot be used in a pattern as it is an alias for function " <> showQualified showIdent fn <> "."
        ...
        renderSimpleErrorMessage (CannotDeriveNewtypeForData tyName) =
          paras
            [ line $ "Cannot derive an instance of the " <> markCode "Newtype" <> " class for non-newtype " <> markCode (runProperName tyName) <> "."
            ]
        renderSimpleErrorMessage (ExpectedWildcard tyName) =
          paras
            [ line $ "Expected a type wildcard (_) when deriving an instance for " <> markCode (runProperName tyName) <> "."
            ]
        renderSimpleErrorMessage (CannotUseBindWithDo name) =
          paras
            [ line $ "The name " <> markCode (showIdent name) <> " cannot be brought into scope in a do notation block, since do notation uses the same name."
            ]
        ...
        renderSimpleErrorMessage (UserDefinedWarning msgTy) =
          let msg = fromMaybe (prettyType msgTy) (toTypelevelString msgTy)
           in paras
                [ line "A custom warning occurred while solving type class constraints:"
                , indent msg
                ]
        ...
        renderSimpleErrorMessage (RoleMismatch var inferred declared) =
          paras
            ...
            , indent . line $
                "The annotation says "
                  <> markCode (displayRole declared)
                  <> " but the role "
                  <> markCode (displayRole inferred)
                  <> " is required."
            ]
          ...
          paras
            ...
            , line $
                "To derive this class, make sure that these variables are only used as the final arguments to type constructors, "
                  ...
                  <> "and that those type constructors themselves have instances of "
                  <> commasAndConjunction "or" (markCode . showQualified runProperName <$> relatedClasses)
                  <> "."
            ]
        ...
        renderSimpleErrorMessage (CannotApplyExpressionOfTypeOnType tyFn tyAr) =
          ...
          where
            infoLine =
              if isMonoType tyFn
                then ["An expression of monomorphic type:"]
                else
        ...
        renderHint (ErrorInModule mn) detail =
          paras
            [ line $ "in module " <> markCode (runModuleName mn)
            , detail
            ]
        ...
        renderHint (ErrorInferringKind ty) detail =
          paras
            ...
            , Box.hsep
                1
                Box.top
                [ line "while inferring the kind of"
                , markCodeBox $ typeAsBox prettyDepth ty
                ]
            ]
        ...
        renderHint (PositionedError srcSpan) detail =
          paras
            [ line $ "at " <> displaySourceSpan relPath (NEL.head srcSpan)
            , detail
            ]
