...
import Language.PureScript.PSString (PSString, decodeString, prettyPrintString)
...
matchType tro = buildPrettyPrinter operators (matchTypeAtom tro)
  where
    operators :: OperatorTable () PrettyPrintType Box
    operators =
      OperatorTable
        [ [Wrap kindArg $ \_ ty -> text "@" <> ty]
...
prettyPrintLabel (Label s) =
  case decodeString s of
    Just s'
      | not (objectKeyRequiresQuoting s') ->
          s'
    _ ->
      prettyPrintString s
