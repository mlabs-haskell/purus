...
tco :: AST -> AST
tco = flip evalState 0 . everywhereTopDownM convert
  where
    tcoVar :: Text -> Text
    ...
    findTailPositionDeps (ident, arity) = allInTailPosition
      where
    ...
    toLoop trFns ident arity outerArgs innerArgs js = do
      ...
      let
              ...
              let
               ...
               in
                Block ss $
                  zipWith
                    ( \val arg ->
                        Assignment ss (Var ss (tcoVar arg)) val
                    )
                    allArgumentValues
                    outerArgs
                    ++ zipWith
                      ( \val arg ->
                          Assignment ss (Var ss (copyVar arg)) val
                      )
                      (drop (length outerArgs) allArgumentValues)
                      innerArgs
                    ++ [ReturnNoResult ss]
        ...
        loopify (VariableIntroduction ss f (Just (p, fn@(Function _ Nothing _ _))))
          ...
          , f `S.member` trFns =
              VariableIntroduction ss f (Just (p, replace (loopify body)))
        loopify other = other
        ...
        Block rootSS $
          map (\arg -> VariableIntroduction rootSS (tcoVar arg) (Just (UnknownEffects, Var rootSS (copyVar arg)))) outerArgs
            ++ [ VariableIntroduction rootSS tcoDone (Just (UnknownEffects, BooleanLiteral rootSS False))
               ...
               , While
                  rootSS
                  (Unary rootSS Not (Var rootSS tcoDone))
                  ( Block
                      rootSS
