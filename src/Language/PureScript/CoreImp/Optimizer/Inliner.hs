...
import Language.PureScript.CoreImp.Optimizer.Common (applyAll, isReassigned, isRebound, isUpdated, removeFromBlock, replaceIdent, replaceIdents, pattern Ref)
...
inlineCommonOperators expander =
  ...
  where
    ...
    mkFn' mkFn_ res 0 = convert
      where
        convert :: AST -> AST
        convert (App _ (Ref mkFnN) [Function s1 Nothing [_] (Block s2 [Return s3 js])])
          | isNFn mkFn_ 0 mkFnN =
              res s1 s2 s3 [] js
        convert other = other
    mkFn' mkFn_ res n = convert
      where
        convert :: AST -> AST
    ...
    runFn' runFn_ res n = convert
      where
        convert :: AST -> AST
        convert js = fromMaybe js $ go n [] js

        go :: Int -> [AST] -> AST -> Maybe AST
        go 0 acc (App ss (Ref runFnN) [fn])
          | isNFn runFn_ n runFnN && length acc == n =
              Just $ res ss fn acc
