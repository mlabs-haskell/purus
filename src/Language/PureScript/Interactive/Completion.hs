...
import Data.List (isInfixOf, isPrefixOf, isSuffixOf, nub, sortBy, stripPrefix)
...
getIdentNames = do
  ...
  return . nub $
    map (T.unpack . P.showQualified P.showIdent) importedVals
...
getTypeNames = do
  ...
  return . nub $
    map (T.unpack . P.showQualified P.runProperName) importedTypes
