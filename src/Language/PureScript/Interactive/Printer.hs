...
printModuleSignatures moduleName P.Environment {..} =
  ...
  where
    printModule's showF = Box.vsep 1 Box.left . showF

    findNameType ::
      M.Map (P.Qualified P.Ident) (P.SourceType, P.NameKind, P.NameVisibility) ->
      P.Qualified P.Ident ->
    ...
    showType ::
      ...
      M.Map (P.Qualified (P.ProperName 'P.TypeName)) ([(Text, P.SourceType)], P.SourceType) ->
      (P.Qualified (P.ProperName 'P.TypeName), Maybe (P.SourceType, P.TypeKind)) ->
      Maybe Box.Box
                ...
                case pt of
                  [(dtProperName, _)] ->
                    case M.lookup (P.Qualified modul dtProperName) dataConstructorsEnv of
                      ...
                      _ -> "data"
                  _ -> "data"
           in Just $ textT (prefix <> " " <> P.runProperName name <> foldMap ((" " <>) . (\(v, _, _) -> v)) typevars) Box.// printCons pt
        _ ->
          Nothing
      where
        printCons pt =
