    ...
    lintDeclaration = tell . f
      where
        ...
        stepE s (Let _ ds' _) = foldMap go ds'
          where
            go d
              | Just i <- getDeclIdent d
              , inScope i s =
                  errorMessage' (declSourceSpan d) (ShadowedName i)
              | otherwise = mempty
        stepE _ _ = mempty
      ...
      where
        ...
        findUnused = go set
          where
            ...
            go unused (TypeVar _ v _) = (S.delete v unused, mempty)
            go unused (ForAll _ _ tv k t1 _) =
              let (nowUnused, errors) = go unused k `combine` go (S.insert tv unused) t1
      ...
      where
        ...
        go (Abs binder v1) =
          let newNames = S.fromList (binderNamesWithSpans binder)
           in removeAndWarn newNames $ go v1
        ...
        go (Case vs alts) =
          let f (CaseAlternative binders gexprs) =
                let bindNewNames = S.fromList (concatMap binderNamesWithSpans binders)
                 ...
                 in removeAndWarn bindNewNames $ mconcat $ map go allExprs
           in mconcat $ map go vs ++ map f alts
        go (TypedValue _ v1 _) = go v1
        go (Do _ es) = doElts es Nothing
        go (Ado _ es v1) = doElts es (Just v1)
        go (Literal _ (ListLiteral es)) = mconcat $ map go es
          ...
          let
            onDecl d (accErrs, accLetNamesRec) =
              let (letNames, recNames) = declIdents d
               ...
               in (errs', accLetNamesRec <> recNames)
        ...
        underDecl (ValueDecl _ _ _ binders gexprs) =
          let bindNewNames = S.fromList (concatMap binderNamesWithSpans binders)
              allExprs = concatMap unguard gexprs
           in removeAndWarn bindNewNames $ foldr1 (<>) $ map go allExprs
        ...
        removeAndWarn newNamesWithSpans (used, errors) =
          let newNames = S.map snd newNamesWithSpans
           ...
           in (filteredUsed, combinedErrors)
