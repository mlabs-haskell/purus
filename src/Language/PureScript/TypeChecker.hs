...
import Control.Monad (forM, unless, void, when, zipWithM_)
...
addTypeSynonym moduleName name args ty kind = do
  ...
  putEnv $
    env
      { types = M.insert qualName (kind, TypeSynonym) (types env)
...
addTypeClass _ qualifiedClassName args implies dependencies ds kind = do
  ...
  putEnv $
    env
      { types = M.insert qualName (kind, ExternData (nominalRolesForKind kind)) (types env)
...
addTypeClassDictionaries mn entries =
  ...
  where
    insertState st = M.insertWith (M.unionWith (M.unionWith (<>))) mn entries (typeClassDictionaries . checkEnv $ st)
...
checkTypeClassInstance ::
  (MonadState CheckState m, MonadError MultipleErrors m) =>
  TypeClassData ->
  -- | index of type class argument
  Int ->
  SourceType ->
  m ()
checkTypeClassInstance cls i = check
  where
  ...
  where
    ...
    go d@(DataBindingGroupDeclaration tys) = do
      ...
      warnAndRethrow (addHint (ErrorInDataBindingGroup bindingGroupNames) . addHint (PositionedError sss)) $ do
        ...
        let dataDeclsWithKinds =
              zipWith
                ( \(dtype, (_, name, args, _)) (dataCtors, ctorKind) ->
                    (dtype, name, args `withKinds` ctorKind, dataCtors, ctorKind)
                )
                dataDecls
                data_ks
        inferRoles' <- fmap (inferDataBindingGroupRoles env moduleName roleDecls)
          . forM dataDeclsWithKinds
          $ \(_, name, args, dataCtors, _) ->
    ...
    checkTypesAreExported ref = checkMemberExport findTcons ref
      where
        ...
        findTcons = everythingOnTypes (++) go
          where
            go (TypeConstructor _ (Qualified (ByModuleName mn') name))
              | mn' == mn =
      ...
      | otherwise = do
          ...
          unless (null missingDataConstructorsNames) $
            throwError . errorMessage' ss' $
              TransitiveDctorExportError dr missingDataConstructorsNames
      where
