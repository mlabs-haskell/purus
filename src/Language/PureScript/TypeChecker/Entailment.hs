...
import Language.PureScript.AST (Binder (..), ErrorMessageHint (..), Expr (..), Literal (..), everythingOnValues, everywhereOnValuesTopDownM, nullSourceAnn, nullSourceSpan, pattern NullSourceSpan)
import Language.PureScript.AST.Declarations (UnknownsHint (..))
import Language.PureScript.Constants.Libs qualified as C
import Language.PureScript.Constants.Prim qualified as C
import Language.PureScript.Crash (internalError)
import Language.PureScript.Environment (Environment (..), FunctionalDependency (..), TypeClassData (..), dictTypeName, kindRow, kindType, mkRecordT, tyBoolean, tyInt, tyString)
import Language.PureScript.Errors (MultipleErrors, SimpleErrorMessage (..), addHint, addHints, errorMessage, rethrow)
import Language.PureScript.Label (Label (..))
import Language.PureScript.Names (Ident (..), ModuleName, ProperName (..), ProperNameType (..), Qualified (..), QualifiedBy (..), byMaybeModuleName, coerceProperName, disqualify, freshIdent, getQual, pattern ByNullSourcePos)
...
data Reflectable
  = -- | For type-level numbers
    ReflectableInt Integer
  | -- | For type-level strings
    ReflectableString PSString
  | -- | For type-level booleans
    ReflectableBoolean Bool
  | -- | For type-level orderings
...
entails SolverOptions {..} constraint context hints =
  ...
  where
        ...
        go work hints' con@(Constraint _ className' kinds' tys' conInfo) = WriterT . StateT . (withErrorMessageHint (ErrorSolvingConstraint con) .) . runStateT . runWriterT $ do
          ...
          case solution of
            Solved substs tcd -> do
              ...
              let match =
                    foldr
                      (\(className, index) dict -> subclassDictionaryValue dict className index)
                      initDict
                      (tcdPath tcd)
          ...
          where
            ...
            mkDictionary (IsSymbolInstance sym) _ =
              let fields = [("reflectSymbol", Abs (VarBinder nullSourceSpan UnusedIdent) (Literal nullSourceSpan (StringLiteral sym)))]
               in return $ App (Constructor nullSourceSpan (coerceProperName . dictTypeName <$> C.IsSymbol)) (Literal nullSourceSpan (ObjectLiteral fields))
            mkDictionary (ReflectableInstance ref) _ =
              let fields = [("reflectType", Abs (VarBinder nullSourceSpan UnusedIdent) (asExpression ref))]
               in pure $ App (Constructor nullSourceSpan (coerceProperName . dictTypeName <$> C.Reflectable)) (Literal nullSourceSpan (ObjectLiteral fields))
            ...
            unknownsInAllCoveringSets indexToArgText tyClassMembers tyArgs coveringSets = do
              ...
              if all (\s -> any (`S.member` s) unkIndices) coveringSets
                then fromMaybe Unknowns unknownsRequiringVtas
                else NoUnknowns
              where
                  ...
                  let
                    ...
                    tyClassMemberVta = M.fromList $ mapMaybe qualifyAndFilter tyClassMembers
                      where
                        ...
                        qualifyAndFilter (ident, _, mbVtaRequiredArgs) =
                          mbVtaRequiredArgs <&> \vtaRequiredArgs ->
                    ...
                    tyClassMembersInExpr = getVars
                      where
                        ...
                        getVarIdents = \case
                          Var _ ident
                            | Just vtas <- M.lookup ident tyClassMemberVta ->
                                [(ident, vtas)]
                          _ ->
                            []
    ...
    solveCoercible env ctx kinds [a, b] = do
      let coercibleDictsInScope = findDicts ctx C.Coercible ByNullSourcePos
          givens = flip mapMaybe coercibleDictsInScope $ \case
            dict
              | [a', b'] <- tcdInstanceTypes dict -> Just (a', b')
              | otherwise -> Nothing
      GivenSolverState {inertGivens} <-
        execStateT (solveGivens env) $
          initialGivenSolverState givens
      (WantedSolverState {inertWanteds}, hints') <-
        runWriterT . execStateT (solveWanteds env) $
    ...
    consSymbol _ _ arg@(TypeLevelString _ s) = do
      ...
      where
        mkTLString = srcTypeLevelString . mkString
    ...
    solveIntCompare ctx args@[a, b, _] = do
      let compareDictsInScope = findDicts ctx C.IntCompare ByNullSourcePos
          givens = flip mapMaybe compareDictsInScope $ \case
            dict
              | [a', b', c'] <- tcdInstanceTypes dict -> mkRelation a' b' c'
              | otherwise -> Nothing
      ...
      where
        ...
        (canMakeProgress, lOut, rOut, uOut, cons, vars) =
          case rest of
            ...
            -- from the output.
            _
              | (right, rightu@(REmptyKinded _ _)) <- rowToList r
    ...
    rowToRowList kind r =
      ...
      where
        ...
        rowListCons (RowListItem _ lbl ty) tl =
          foldl
            srcTypeApp
            (srcKindApp (srcTypeConstructor C.RowListCons) kind)
...
matches deps TypeClassDictionaryInScope {..} tys =
  ...
  let matched = zipWith typeHeadsAreEqual tys tcdInstanceTypes
   in -- Now, use any functional dependencies to infer any remaining types
  ...
  where
    ...
    verifySubstitution mts = foldMap meet mts $> mts
      where
...
newDictionaries path name (Constraint _ className instanceKinds instanceTy _) = do
  ...
  supDicts <-
    join
      <$> zipWithM
        ( \(Constraint ann supName supKinds supArgs _) index ->
            let sub = zip (map fst typeClassArguments) instanceTy
             in newDictionaries
                  ((supName, index) : path)
                  name
                  ( Constraint
...
mkContext = foldr combineContexts M.empty . map fromDict
  where
