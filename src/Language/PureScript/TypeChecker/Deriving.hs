...
-- HLint doesn't recognize that TypeApplications is used in a pattern
{-# LANGUAGE GADTs #-}
...
import Language.PureScript.Names (Ident (..), ModuleName (..), Name (..), ProperName (..), ProperNameType (..), Qualified (..), QualifiedBy (..), coerceProperName, freshIdent, qualify, pattern ByNullSourcePos)
...
import Language.PureScript.Types (Constraint (..), SourceType, Type (..), completeBinderList, eqType, everythingOnTypes, replaceAllTypeVars, srcTypeVar, usedTypeVariables, pattern REmptyKinded)
...
deriveInstance instType className strategy = do
  ...
  case strategy of
    ...
    NewtypeStrategy ->
      case tys of
        _ : _
          | Just utc <- unwrapTypeConstructor (last tys)
                ...
                then do
                  ...
                  for_ (extractNewtypeName mn tys) $ \nm -> do
                    unless (hasNewtypeSuperclassInstance constraintClass' nm (typeClassDictionaries env)) $
                      tell . errorMessage $
                        MissingNewtypeSuperclassInstance constraintClass className tys
          ...
          lookIn mn' =
            elem nt
              . ( toList
                    ...
                    . M.elems
                    <=< toList
                    . (M.lookup su <=< M.lookup (ByModuleName mn'))
                )
              $ dicts
  ...
  where
    ...
    mkCtorClauses ((ctorName, tys), isLast) = do
      ...
      let tests = zipWith3 toOrdering (map mkVar identsL) (map mkVar identsR) tys'
          extras
            | not isLast =
                [ CaseAlternative [nullCaseBinder, NullBinder] (unguarded (orderingCtor "LT"))
    ...
    appendAll = \case
      ...
      (x : xs) ->
        Case
          [x]
          [ CaseAlternative [orderingBinder "LT"] (unguarded (orderingCtor "LT"))
...
decomposeRec = fmap (sortOn fst) . go
  where
    go (RCons _ str typ typs) = fmap ((str, typ) :) (go typs)
...
decomposeRec' = sortOn fst . go
  where
    go (RCons _ str typ typs) = (str, typ) : go typs
    go _ = []
...
validateParamsInTypeConstructors derivingClass utc isBi CovariantClasses {..} contravarianceSupport = do
  ...
  where
    ...
    typeToUsageOf tcds subst = fix $ \go params isNegative ->
      let
        ...
        tryBiClasses ht tyLArg tyArg
          ...
          | Just (ContravarianceSupport c _ _ ContravariantClasses {..}) <- contravarianceSupport
          , hasInstance tcds ht proClass =
        ...
        tryMonoClasses ht tyArg
          ...
          | Just (ContravarianceSupport c _ _ ContravariantClasses {..}) <- contravarianceSupport
          , hasInstance tcds ht contraClass =
...
mkCasesForTraversal mn handleArg extractExpr ctors = do
  ...
  fmap (lamCase m) . for ctors $ \(ctorName, ctorUsages) -> do
    ...
    fmap (CaseAlternative [caseBinder] . unguarded) . extractExpr $
      fmap (foldl' App ctor) . for ctorArgs $
        \(ident, mbUsage) -> maybe pure handleArg mbUsage $ mkVar ident
...
deriveFunctor mbLParamIsContravariant paramIsContravariant mapName nm utc = do
  ctors <-
    validateParamsInTypeConstructors nm utc isBi functorClasses $
      Just $
