...
import Language.PureScript.Names (
  ...
  Ident (Ident, UnusedIdent), runIdent, coerceProperName, disqualify
  )
import Language.PureScript.CoreFn.TypeLike (TypeLike(applyType, funTy), quantify)
import Language.PureScript.Types (
  Type (TypeConstructor, TypeApp, REmpty, RCons, TypeVar), SourceType
  )
...
ledgerConstructorsEnvReadable = docString
                                . (<> hardline)
                                . vcat
                                . punctuate hardline
