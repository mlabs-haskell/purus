...
import Control.Lens.Combinators (preview, transform)
...
import Language.Purus.Pretty (prettyTypeStr)
import Language.PureScript.CoreFn.TypeLike
import Language.PureScript.Environment (mkTupleTyName, pattern (:->))
...
mkTypeBindDict _datatypes main =
  doTraceM "mkTypeBindDict" (prettyStr main) >> case runState act initState of
    (Left err, _) -> Left err
  ...
  CaseE resTy scrut (UnguardedAlt _ (LitP patLit) rhs : alts) -> do
    ...
    pure $
      CaseE
        resTy
        eqTest
        [ UnguardedAlt M.empty trueP rhs
